...
namespace tool
{
	class TimelineRenderer
	{
		...
		private readonly Arguments _parsedArgs;

		public TimelineRenderer(Arguments parsedArgs)
		{
			_parsedArgs = parsedArgs;
		}

		internal void Render(TimelineData timelineDate, XmlNode node)
		{
			var z = timelineDate.WorkdayInterval;
			var f = timelineDate.Intervals;
			...
			using (var bmp = new Bitmap(barRect.Width + 2 * sideOffset, barRect.Height + topOffset + bottomOffset + legendHeight))
			{
				using (var gr = Graphics.FromImage(bmp))
				{
					...
					for (int i = 0; i < count; i++)
					{
						...
						string[] values = a.Task.Node.GetParents(u => u == node)
				...
				bmp.Save(_parsedArgs.OutputBitmap);
			}
		}
